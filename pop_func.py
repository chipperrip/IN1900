...
# function for calculating N values, we already know that the
# calculation works from the previous assignment, no need to test
def population (t, k, B, C):
    return B / (1+C*exp(-k*t))


# list of N values calculated for each t
N = [population(t[i], k, B, C) for i in range(n+1)]
